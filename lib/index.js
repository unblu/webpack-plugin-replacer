...
class Replacer {
    constructor(opts) {
        console.log("Unblu WebpackPluginValidator created.");
    ...
    replaceInChunks(compilation, chunks) {
        chunks.reduce((acc, chunk) => acc.concat(chunk.files || []), [])
            ...
            .forEach((file) => {
                console.log("Unblu WebpackPluginValidator file: ", file);
    ...
    apply(compiler) {
        compiler.hooks.compilation.tap('WebpackPluginValidator', (compilation) => {
            if (this.options.sourceMap) {
                compilation.hooks.buildModule.tap('WebpackPluginValidator', (moduleArg) => {
            ...
            if (this.options.target === TARGET_MODULES) {
                compilation.hooks.optimizeModules.tap('WebpackPluginValidator', (modules) => {
                ...
                compilation.hooks.optimizeChunkAssets.tap('WebpackPluginValidator', (chunks) => {
                    this.replaceInChunks(compilation, chunks);
                });
            } else if(this.options.target === TARGET_AFTER_CHUNKS) {
                compilation.hooks.afterOptimizeChunkAssets.tap('WebpackPluginValidator', (chunks) => {
                    this.replaceInChunks(compilation, chunks);
                });
            } else {
